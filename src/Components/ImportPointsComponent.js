...
function ImportPointsComponent(props) {
  ...
  useEffect(() => {
    if (selectedRoomTypeId) {
      axios.get('http://localhost:3001/dvc-calc-api/viewTypes/' + selectedRoomTypeId).then(resp => {
  ...
  useEffect(() => {
    if (selectedViewTypeId) {
      axios.get('http://localhost:3001/dvc-calc-api/pointValue/' + selectedViewTypeId).then(resp => {
  ...
  const savePointValues = () => {
    axios.post('http://localhost:3001/dvc-calc-api/pointValue', { pointValues: formatPointValuesForSave(pointValues) }).then(resp => {
